...
// ---------- speex_jni native build ----------
// Bundle the libspeex C sources from android/app/src/main/cpp/ and compile
// them into speex_jni.dll via MinGW gcc (x86_64-w64-mingw32). Same JNA shim
// as macOS/Linux ports.
//
// Compiler resolution order:
//   1. RCFORB_MINGW_GCC env var (full path)
//   2. x86_64-w64-mingw32-gcc on PATH (Linux/macOS cross-build hosts)
//   3. gcc on PATH (Windows MSYS2/MinGW64 host)
...
val dllFile = file("$nativeOutDir/speex_jni.dll")
...
val buildSpeexJni by tasks.registering {
    ...
    doLast {
        ...
        fun resolveCompiler(): String {
            System.getenv("RCFORB_MINGW_GCC")?.let { return it }
            val candidates = listOf("x86_64-w64-mingw32-gcc", "gcc")
            for (c in candidates) {
                val r = ProcessBuilder("which", c).redirectErrorStream(true).start()
                val out = r.inputStream.bufferedReader().readText().trim()
                if (r.waitFor() == 0 && out.isNotEmpty()) return out
            }
            throw GradleException(
                "No MinGW gcc found. Install mingw-w64 (apt/brew) or run from MSYS2 mingw64 shell on Windows. " +
                "Override with RCFORB_MINGW_GCC=/path/to/gcc."
            )
        }
        ...
        val args = mutableListOf(
            ...
            "-Wl,--out-implib,${dllFile.absolutePath}.a",
            "-o", dllFile.absolutePath
        )
        sources.forEach { args.add("$androidCppDir/$it") }

        // JNA-friendly shim wrapping speex_jni internals (mirrors macOS/Linux).
        ...
        println("Building speex_jni.dll via $gcc...")
        val proc = ProcessBuilder(argsNoShim).redirectErrorStream(true).start()
        val output = proc.inputStream.bufferedReader().readText()
        val rc = proc.waitFor()
        if (rc != 0) {
            throw GradleException("MinGW gcc failed (exit $rc):\n$output")
        }
        println("Built: ${dllFile.absolutePath}")
    }
}

tasks.processResources {
    dependsOn(buildSpeexJni)
    from(dllFile) { into("native") }
}
...
compose.desktop {
    application {
        ...
        nativeDistributions {
            ...
            // time, so we only declare Windows formats when actually on
            // Windows. Keeps the file parseable on any host (matches Linux).
            ...
            windows {
                // .ico is generated from icon/AppIcon.png by the build script
                // before gradle runs. Guard the set() so `./gradlew run` still
                // works during local dev when the .ico hasn't been generated.
                val icoFile = project.file("icon/AppIcon.ico")
                if (icoFile.exists()) iconFile.set(icoFile)
                packageVersion = "1.0.0"
                menuGroup = "RCForb"
                // Stable upgrade UUID — must not change between releases or
                // Windows treats new versions as separate products.
                upgradeUuid = "8a4cae2b-1bf6-4b3e-9c4e-9e4f9b02a751"
                shortcut = true
                dirChooser = true
